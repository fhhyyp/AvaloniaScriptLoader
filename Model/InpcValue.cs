using ScriptLang.Runtime;

namespace AvaloniaScriptLoader.Model;

/// <summary>
/// 可观察值 — 模拟 INotifyPropertyChanged 接口
///
/// 包装一个 Value，当值通过 Set() 变更时自动通知所有订阅者。
/// 与 PropertyBinder 配合实现脚本变量 → UI 控件的自动数据绑定。
///
/// 脚本用法:
///   var name = inpc("张三")
///   label({"text" = name})        // 自动绑定
///   name.set("李四")              // 值变更 → UI 自动更新
/// </summary>
public class InpcValue
{
    private Value _value;
    private readonly List<Action<Value>> _subscribers = [];

    /// <summary>
    /// 创建可观察值
    /// </summary>
    /// <param name="initialValue">初始值</param>
    public InpcValue(Value initialValue)
    {
        _value = initialValue;
    }

    /// <summary>
    /// 获取当前值
    /// </summary>
    public Value Get()
    {
        return _value;
    }

    /// <summary>
    /// 设置新值并通知所有订阅者。
    /// 若新旧值相等（按 IsEqual 语义），跳过通知。
    /// </summary>
    /// <param name="newValue">新值</param>
    /// <param name="forceNotify">强制通知（即使值相同）</param>
    public void Set(Value newValue, bool forceNotify = false)
    {
        bool changed = forceNotify || !ValuesEqual(_value, newValue);
        _value = newValue;

        if (changed)
        {
            NotifySubscribers();
        }
    }

    /// <summary>
    /// 原地更新数值（用于 MutableNumber 场景，如 count = count + 1）。
    /// 更新后通知所有订阅者。
    /// </summary>
    /// <param name="updater">接收当前值，返回新值</param>
    public void Update(Func<Value, Value> updater)
    {
        var newValue = updater(_value);
        Set(newValue);
    }

    /// <summary>
    /// 注册变更回调。回调在 Set() 的调用线程上执行。
    /// </summary>
    /// <param name="callback">接收新值的回调</param>
    public void OnChange(Action<Value> callback)
    {
        _subscribers.Add(callback);
    }

    /// <summary>
    /// 移除变更回调
    /// </summary>
    public void RemoveCallback(Action<Value> callback)
    {
        _subscribers.Remove(callback);
    }

    /// <summary>
    /// 获取订阅者数量（调试用）
    /// </summary>
    public int SubscriberCount => _subscribers.Count;

    // ========================================================================
    // 内部方法
    // ========================================================================

    private void NotifySubscribers()
    {
        // 快照遍历，防止回调中修改订阅列表
        var snapshots = _subscribers.ToArray();
        foreach (var callback in snapshots)
        {
            try
            {
                callback(_value);
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine(
                    $"[InpcValue] 通知订阅者异常: {ex.Message}");
            }
        }
    }

    /// <summary>
    /// 值相等判断。对于不可变类型（数字/字符串/bool/null）进行值比较，
    /// 对于可变类型（ArrayValue/ObjectValue）始终返回 false（不等），
    /// 因为调用者显式调用了 set()，应触发通知。
    /// </summary>
    private static bool ValuesEqual(Value a, Value b)
    {
        // 可变容器类型：即使同一个对象引用，内部数据也可能已变更，始终通知
        if (a.IsArray || b.IsArray) return false;
        if (a.IsObject || b.IsObject) return false;

        if (a.IsNull && b.IsNull) return true;

        if (a.IsNumber && b.IsNumber)
        {
            try
            {
                double da = a.As<double>();
                double db = b.As<double>();
                return Math.Abs(da - db) < 1e-10;
            }
            catch
            {
                return false;
            }
        }
        if (a.IsString && b.IsString)
            return a.AsString() == b.AsString();
        if (a.IsBool && b.IsBool)
            return a.AsBool() == b.AsBool();

        // 引用相等（对于 ArrayValue / ObjectValue）
        return ReferenceEquals(a, b);
    }
}
